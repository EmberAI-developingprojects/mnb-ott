...
function LibraryContent() {
  ...
  /* Жанрын filter солигдох бүрд visible-ийг reset */
  useEffect(() => { setVisible(PAGE_SIZE); }, [genreFilter]);

  useEffect(() => {
    api.get<{ success: true; data: { videos: Video[] } }>("/api/vod/library", { params: { all: 1 } })
      ...
      {loading
        ...
        : sections.map((s) => (
            <GenreRow key={s.genre} title={s.genre} items={s.items}
              seeAllHref={`/library?g=${encodeURIComponent(s.genre)}`} seeAllLabel={t("see_all_short")} />
          ))
      }
    </div>
  );
